...
function getStyleSettings() {
	return {
		...
		keepOpen: localStorage.getItem('keepOpen') || 'db-hidden'
	};
}
...
function initDebugUI() {
	...
	container.innerHTML = `
		...
		<div id="debug-window" class="${keepOpen}" style="display: ${keepOpen ? 'block' : 'none'};">
			<div id="debug-tabs">
        ...
        <div class="control-field">
			<label>Keep Open:</label>
				<select name="keepOpen">
					<option value="db-hidden" ${!current.keepOpen ? 'selected' : ''}>No</option>
					<option value="db-visible" ${current.keepOpen ? 'selected' : ''}>Yes</option>
				</select>
        </div>
